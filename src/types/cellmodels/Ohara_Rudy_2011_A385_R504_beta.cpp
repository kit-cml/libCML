...
#include "Ohara_Rudy_2011_A385_R504_beta.hpp"
#include <cmath>
#include <cstdlib>
...
Ohara_Rudy_2011_A385_R504_beta::Ohara_Rudy_2011_A385_R504_beta()
{
	algebraic_size = 198;
	constants_size = 144;
	states_size = 41;
...
void Ohara_Rudy_2011_A385_R504_beta::___initConsts(double type)
{
...
STATES[hf] = 1;
STATES[hs] = 1;
//CONSTANTS[GNa] = 75*(939./2000.);
CONSTANTS[GNa] = 35.2125;
STATES[j] = 1;
STATES[hsp] = 1;
STATES[jp] = 1;
STATES[mL] = 0;
...
STATES[hL] = 1;
STATES[hLp] = 1;
//CONSTANTS[GNaL_b] = 0.0075*(93.9/200.);
CONSTANTS[GNaL_b] = 0.003521;
...
void Ohara_Rudy_2011_A385_R504_beta::___applyDutta()
{
...
void Ohara_Rudy_2011_A385_R504_beta::___applyDrugEffect(double conc, const double *hill)
{
...
void Ohara_Rudy_2011_A385_R504_beta::initConsts()
{
	___initConsts(0.);
}

void Ohara_Rudy_2011_A385_R504_beta::initConsts(double type)
{
	___initConsts(type);
}

void Ohara_Rudy_2011_A385_R504_beta::initConsts(double type, double conc, const double *hill, bool is_dutta)
{
  ___initConsts(type);
  mpi_printf(0,"Celltype: %lf\n", CONSTANTS[celltype]);
  mpi_printf(0,"Control %lf %lf %lf %lf %lf\n", CONSTANTS[PCa_b], CONSTANTS[GK1_b], CONSTANTS[GKs_b], CONSTANTS[GNaL_b], CONSTANTS[GKr_b]);
  if(is_dutta == true) ___applyDutta();
  mpi_printf(0,"After Dutta %lf %lf %lf %lf %lf\n", CONSTANTS[PCa_b], CONSTANTS[GK1_b], CONSTANTS[GKs_b], CONSTANTS[GNaL_b], CONSTANTS[GKr_b]);
}

void Ohara_Rudy_2011_A385_R504_beta::computeRates( double TIME, double *CONSTANTS, double *RATES, double *STATES, double *ALGEBRAIC )
{
CONSTANTS[cmdnmax] = (CONSTANTS[celltype]==1.00000 ?  CONSTANTS[cmdnmax_b]*1.30000 : CONSTANTS[cmdnmax_b]);
CONSTANTS[GNaL] = (CONSTANTS[celltype]==1.00000 ?  CONSTANTS[GNaL_b]*0.600000 : CONSTANTS[GNaL_b]);
CONSTANTS[Gto] = (CONSTANTS[celltype]==1.00000 ?  CONSTANTS[Gto_b]*4.00000 : CONSTANTS[celltype]==2.00000 ?  CONSTANTS[Gto_b]*4.00000 : CONSTANTS[Gto_b]);
...
ALGEBRAIC[thf] = 1.55000/( 1.43200e-05*exp(- (STATES[V]+1.19600)/6.28500)+ 6.14900*exp((STATES[V]+0.509600)/20.2700));
ALGEBRAIC[ths] = 1.06000/( 0.00979400*exp(- (STATES[V]+17.9500)/28.0500)+ 0.334300*exp((STATES[V]+5.73000)/56.6600));
...
void Ohara_Rudy_2011_A385_R504_beta::solveAnalytical(double dt)
{
#ifdef EULER
...
double Ohara_Rudy_2011_A385_R504_beta::set_time_step(double TIME,
  double time_point,
  ...
  if (TIME <= time_point || (TIME - floor(TIME / CONSTANTS[BCL]) * CONSTANTS[BCL]) <= time_point) {
    ...
    if (std::abs(RATES[V] * time_step) <= min_dV) {//Slow changes in V
        ...
        while (std::abs(RATES[V] * time_step) >= max_dV &&
