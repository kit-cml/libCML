...
CONSTANTS[stim_start] is i_Stim_Start in component membrane (millisecond).
CONSTANTS[stim_end] is i_Stim_End in component membrane (millisecond).
CONSTANTS[amp] is i_Stim_Amplitude in component membrane (microA_per_microF).
CONSTANTS[BCL] is i_Stim_Period in component membrane (millisecond).
CONSTANTS[duration] is i_Stim_PulseDuration in component membrane (millisecond).
...
STATES[cli] is cli in component intracellular_ions (millimolar).
STATES[clss] is clss in component intracellular_ions (millimolar).
...
ALGEBRAIC[JdiffCl] is JdiffCl in component diff (millimolar_per_millisecond).
...
ALGEBRAIC[ECl] is ECl in component reversal_potentials (millivolt).
ALGEBRAIC[EClss] is EClss in component reversal_potentials (millivolt).
...
CONSTANTS[tauCl] is tauCl in component diff (millisecond).
...
RATES[cli] is d/dt cli in component intracellular_ions (millimolar).
RATES[clss] is d/dt clss in component intracellular_ions (millimolar).
...
Tomek_dynCl::Tomek_dynCl()
{
  algebraic_size = 226;
  constants_size = 162 + 2; // Add 2 constant for scaling factor, Jtr and Jleak
  states_size = 45;
...
void Tomek_dynCl::___initConsts(double type)
{
  ...
  STATES[CaMKt] = 1.095026e-02;
  STATES[cass] = 6.497341e-05;
  ...
  STATES[nai] = 1.239736e+01;
  STATES[nass] = 1.239770e+01;
  STATES[ki] = 1.477115e+02;
  STATES[kss] = 1.477114e+02;
  STATES[cansr] = 1.528001e+00;
  STATES[cajsr] = 1.525693e+00;
  STATES[cai] = 7.453481e-05;
  STATES[cli] = 2.920698e+01;
  STATES[clss] = 2.920696e+01;
  ...
  STATES[m] = 6.517154e-04;
  STATES[h] = 8.473267e-01;
  STATES[j] = 8.471657e-01;
  STATES[hp] = 7.018454e-01;
  STATES[jp] = 8.469014e-01;
  CONSTANTS[GNa] = 11.7802;
  STATES[mL] = 1.351203e-04;
  CONSTANTS[thL] = 200;
  STATES[hL] = 5.566017e-01;
  STATES[hLp] = 3.115491e-01;
  CONSTANTS[GNaL_b] = 0.0279;
  CONSTANTS[Gto_b] = 0.16;
  STATES[a] = 8.899259e-04;
  CONSTANTS[EKshift] = 0;
  STATES[iF] = 9.996716e-01;
  STATES[iS] = 5.988908e-01;
  STATES[ap] = 4.534165e-04;
  STATES[iFp] = 9.996716e-01;
  STATES[iSp] = 6.620692e-01;
  CONSTANTS[Kmn] = 0.002;
  CONSTANTS[k2n] = 500;
  CONSTANTS[PCa_b] = 8.3757e-05;
  STATES[d] = 1.588841e-31;
  CONSTANTS[Aff] = 0.6;
  STATES[ff] = 1.000000e+00;
  STATES[fs] = 9.401791e-01;
  STATES[fcaf] = 1.000000e+00;
  STATES[fcas] = 9.999014e-01;
  STATES[jca] = 9.999846e-01;
  STATES[ffp] = 1.000000e+00;
  STATES[fcafp] = 1.000000e+00;
  STATES[nca_ss] = 4.899378e-04;
  STATES[nca_i] = 8.326009e-04;
  CONSTANTS[tjca] = 72.5;
  CONSTANTS[vShift] = 0;
  CONSTANTS[offset] = 0;
  ...
  STATES[C1] = 9.982511e-01;
  STATES[C2] = 7.936020e-04;
  STATES[C3] = 6.532143e-04;
  STATES[I] = 9.804083e-06;
  STATES[O] = 2.922449e-04;
  CONSTANTS[alpha_1] = 0.154375;
  CONSTANTS[beta_1] = 0.1911;
  CONSTANTS[GKs_b] = 0.0011;
  STATES[xs1] = 2.439590e-01;
  STATES[xs2] = 1.586167e-04;
  ...
  CONSTANTS[tauCl] = 2.0;
  CONSTANTS[bt] = 4.75;
  STATES[Jrel_np] = 1.808248e-22;
  STATES[Jrel_p] = 4.358608e-21;
  ...
  CONSTANTS[Gto] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[Gto_b] * 2.00000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[Gto_b] * 2.00000
                                                                                                                 : CONSTANTS[Gto_b]);
  CONSTANTS[Afs] = 1.00000 - CONSTANTS[Aff];
  CONSTANTS[PCa] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[PCa_b] * 1.20000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[PCa_b] * 2.00000
                                                                                                                 : CONSTANTS[PCa_b]);
  CONSTANTS[Io] = (0.500000 * (CONSTANTS[nao] + CONSTANTS[ko] + CONSTANTS[clo] + 4.00000 * CONSTANTS[cao])) / 1000.00;
  CONSTANTS[GKr] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[GKr_b] * 1.30000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[GKr_b] * 0.800000
                                                                                                                 : CONSTANTS[GKr_b]);
  CONSTANTS[GKs] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[GKs_b] * 1.40000 : CONSTANTS[GKs_b]);
  CONSTANTS[GK1] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[GK1_b] * 1.20000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[GK1_b] * 1.30000
                                                                                                                 : CONSTANTS[GK1_b]);
  ...
  CONSTANTS[Gncx] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[Gncx_b] * 1.10000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[Gncx_b] * 1.40000
                                                                                                                   : CONSTANTS[Gncx_b]);
  ...
  CONSTANTS[Pnak] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[Pnak_b] * 0.900000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[Pnak_b] * 0.700000
                                                                                                                    : CONSTANTS[Pnak_b]);
}

void Tomek_dynCl::___applyCVar(const double *cvar)
{
  ...
  // For testing purpose, directly scaling the conductance, not base
  // CONSTANTS[GNa] *= cvar[0];     // GNa
...
void Tomek_dynCl::___applyDrugEffect(double conc, const double *hill)
{
  ...
  // Testing for single cell purpoose (endo)
...
void Tomek_dynCl::initConsts()
{
  ___initConsts(0.);
}

void Tomek_dynCl::initConsts(double type)
{
  ___initConsts(type);
}

void Tomek_dynCl::initConsts(double type, double conc, const double *hill)
{
  ___initConsts(type);
...
void Tomek_dynCl::initConsts(double type, double conc, const double *hill, const double *cvar)
{
...
void Tomek_dynCl::computeRates(double TIME, double *CONSTANTS, double *RATES, double *STATES, double *ALGEBRAIC)
{
  CONSTANTS[cmdnmax] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[cmdnmax_b] * 1.30000 : CONSTANTS[cmdnmax_b]);
  CONSTANTS[GNaL] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[GNaL_b] * 0.600000 : CONSTANTS[GNaL_b]);
  CONSTANTS[Gto] = (CONSTANTS[celltype] == 1.00000 ? CONSTANTS[Gto_b] * 2.00000 : CONSTANTS[celltype] == 2.00000 ? CONSTANTS[Gto_b] * 2.00000
  ...
  ALGEBRAIC[Iss] = (0.500000 * (STATES[nass] + STATES[kss] + STATES[clss] + 4.00000 * STATES[cass])) / 1000.00;
  ...
  ALGEBRAIC[Istim] = (TIME >= CONSTANTS[stim_start] && (TIME - CONSTANTS[stim_start]) - floor((TIME - CONSTANTS[stim_start]) / CONSTANTS[BCL]) * CONSTANTS[BCL] <= CONSTANTS[duration] ? CONSTANTS[amp] : 0.00000);
  ALGEBRAIC[Ii] = (0.500000 * (STATES[nai] + STATES[ki] + STATES[cli] + 4.00000 * STATES[cai])) / 1000.00;
  ...
  ALGEBRAIC[ECl] = ((CONSTANTS[R] * CONSTANTS[T]) / (CONSTANTS[zcl] * CONSTANTS[F])) * log(CONSTANTS[clo] / STATES[cli]);
  ALGEBRAIC[IClCa_sl] = (((1.00000 - CONSTANTS[Fjunc]) * CONSTANTS[GClCa]) / (1.00000 + CONSTANTS[KdClCa] / STATES[cai])) * (STATES[V] - ALGEBRAIC[ECl]);
  ALGEBRAIC[EClss] = ((CONSTANTS[R] * CONSTANTS[T]) / (CONSTANTS[zcl] * CONSTANTS[F])) * log(CONSTANTS[clo] / STATES[clss]);
  ALGEBRAIC[IClCa_junc] = ((CONSTANTS[Fjunc] * CONSTANTS[GClCa]) / (1.00000 + CONSTANTS[KdClCa] / STATES[cass])) * (STATES[V] - ALGEBRAIC[EClss]);
  ALGEBRAIC[IClCa] = ALGEBRAIC[IClCa_junc] + ALGEBRAIC[IClCa_sl];
  ALGEBRAIC[IClb] = CONSTANTS[GClb] * (STATES[V] - ALGEBRAIC[ECl]);
  ...
  ALGEBRAIC[JdiffCl] = (STATES[clss] - STATES[cli]) / CONSTANTS[tauNa];
  RATES[cli] = ((ALGEBRAIC[IClb] + ALGEBRAIC[IClCa_sl]) * CONSTANTS[Acap]) / (CONSTANTS[F] * CONSTANTS[vmyo]) + (ALGEBRAIC[JdiffCl] * CONSTANTS[vss]) / CONSTANTS[vmyo];
  RATES[clss] = (ALGEBRAIC[IClCa_junc] * CONSTANTS[Acap]) / (CONSTANTS[F] * CONSTANTS[vss]) - ALGEBRAIC[JdiffCl];
...
void Tomek_dynCl::solveAnalytical(double dt)
{
}
