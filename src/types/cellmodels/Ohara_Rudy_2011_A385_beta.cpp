...
#include "Ohara_Rudy_2011_A385_beta.hpp"
#include <cmath>
#include <cstdlib>
...
Ohara_Rudy_2011_A385_beta::Ohara_Rudy_2011_A385_beta()
{
	algebraic_size = 198;
	constants_size = 144;
	states_size = 41;
...
void Ohara_Rudy_2011_A385_beta::___initConsts(double type)
{
...
STATES[hf] = 1;
STATES[hs] = 1;
//CONSTANTS[GNa] = 75*(1519/2000);
CONSTANTS[GNa] = 56.9625;
STATES[j] = 1;
STATES[hsp] = 1;
STATES[jp] = 1;
STATES[mL] = 0;
...
STATES[hL] = 1;
STATES[hLp] = 1;
//CONSTANTS[GNaL_b] = 0.0075*(151.9/200);
CONSTANTS[GNaL_b] = 0.005696;
...
void Ohara_Rudy_2011_A385_beta::___applyDutta()
{
...
void Ohara_Rudy_2011_A385_beta::___applyDrugEffect(double conc, const double *hill)
{
...
void Ohara_Rudy_2011_A385_beta::initConsts()
{
	___initConsts(0.);
}

void Ohara_Rudy_2011_A385_beta::initConsts(double type)
{
	___initConsts(type);
}

void Ohara_Rudy_2011_A385_beta::initConsts(double type, double conc, const double *hill, bool is_dutta)
{
	___initConsts(type);
...
//	if(conc > 0 && hill != NULL) ___applyDrugEffect(conc, hill);
//	mpi_printf(0,"After drug %lf %lf %lf %lf %lf\n", CONSTANTS[PCa], CONSTANTS[GK1], CONSTANTS[GKs], CONSTANTS[GNaL], CONSTANTS[GKr]);
}

void Ohara_Rudy_2011_A385_beta::computeRates( double TIME, double *CONSTANTS, double *RATES, double *STATES, double *ALGEBRAIC )
{
CONSTANTS[cmdnmax] = (CONSTANTS[celltype]==1.00000 ?  CONSTANTS[cmdnmax_b]*1.30000 : CONSTANTS[cmdnmax_b]);
CONSTANTS[GNaL] = (CONSTANTS[celltype]==1.00000 ?  CONSTANTS[GNaL_b]*0.600000 : CONSTANTS[GNaL_b]);
CONSTANTS[Gto] = (CONSTANTS[celltype]==1.00000 ?  CONSTANTS[Gto_b]*4.00000 : CONSTANTS[celltype]==2.00000 ?  CONSTANTS[Gto_b]*4.00000 : CONSTANTS[Gto_b]);
...
ALGEBRAIC[thf] = 2.28000/( 1.43200e-05*exp(- (STATES[V]+1.19600)/6.28500)+ 6.14900*exp((STATES[V]+0.509600)/20.2700));
ALGEBRAIC[ths] = 1.78000/( 0.00979400*exp(- (STATES[V]+17.9500)/28.0500)+ 0.334300*exp((STATES[V]+5.73000)/56.6600));
...
void Ohara_Rudy_2011_A385_beta::solveAnalytical(double dt)
{
#ifdef EULER
...
double Ohara_Rudy_2011_A385_beta::set_time_step(double TIME,
  double time_point,
